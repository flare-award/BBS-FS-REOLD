...
public class UIFilterClip extends UIClip<FilterClip>
{
    ...
    public static IKey getChannelLabel(String id)
    {
        ...
        {
            String prefix = "hsl_" + BBSSettings.HSL_COLOR_IDS[i] + "_";

            if (id.startsWith(prefix))
            {
                String component = id.substring(prefix.length());
                IKey componentLabel = component.equals("hue") ? UIKeys.FILM_FILTERS_HSL_HUE
                    : component.equals("saturation") ? UIKeys.FILM_FILTERS_HSL_SATURATION
                    : UIKeys.FILM_FILTERS_HSL_LIGHTNESS;

                return IKey.constant("%s · %s").format(UIKeys.FILM_FILTERS_HSL_COLORS[i], componentLabel);
            }
        }
